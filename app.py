...
def export_attendance_history():
    ...
    if attempt_filter == 'all':
        ...
        writer.writerow(['T??n h??c sinh', '?i??m', 'Nh??­n x??t'])
...
def edit_test_exam(exam_id):
    ...
    if request.method == 'POST':
        ...
        if not title:
            flash('TÃªn bÃ i kiá»m tra khÃ´ng ÄÆ°á»£c Äá» trá»ng.', 'danger')
            return render_template('test_form.html', available_grades=available_grades, exam=exam, is_edit_mode=True)

        if not grade:
            flash('Báº¡n cáº§n chá»n khá»‘i lá»›p.', 'danger')
            return render_template('test_form.html', available_grades=available_grades, exam=exam, is_edit_mode=True)
        ...
        flash(f'ÄÃ£ cáº­p nháº­t bÃ i kiá»m tra "{title}".', 'success')
